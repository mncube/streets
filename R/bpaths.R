...
bpaths <- function(destination = c(i,j), branch = c(bi, bj)){
  ...
  if (bi == 0 || bj == 0){
    ...
    #   (start from branch coordinate and move one step left then one step up,
    #     from this coordinate compute mpaths to destination))
    mpaths(c(bi, bj-1))*(mpaths(c(i-bi, j-bj)) + mpaths(c(i -(bi-1), j-(bj+1)))) +
      #(mpaths approaching from left of branch)*
      # ((mpaths from branch to destination) +
      #   (start from branch coordinate and move one step down then one step right,
      #     from this coordinate compute mpaths to destination))
      mpaths(c(bi-1,bj))*(mpaths(c(i-bi,j-bj)) + mpaths(c(i-(bi+1), j-(bj-1))))
  }
